...
const CompletionModal = dynamic(
  () => import("@/components/CompletionModal").then((m) => m.CompletionModal),
  { ssr: false }
);
const KeyboardShortcutsModal = dynamic(
  () =>
    import("@/components/puzzle/KeyboardShortcutsModal").then(
      (m) => m.KeyboardShortcutsModal
    ),
  { ssr: false }
);
const ToastContainer = dynamic(
  () => import("@/components/Toast").then((m) => m.ToastContainer),
  { ssr: false }
);
...
export interface PuzzlePageClientProps {
  ...
  initialPuzzleContent: string | null;
}

export default function PuzzlePageClient({
  puzzleId,
  initialPuzzle,
  initialPuzzleContent,
...
function PuzzlePageContent({
  ...
  const [puzzleContent, setPuzzleContent] = useState<string | null>(initialPuzzleContent);
  const [progressData, setProgressData] = useState<ProgressResponse | null>(null);
  const [isLoading, setIsLoading] = useState(!initialPuzzle || !initialPuzzleContent);
  ...
  useEffect(() => {
    const fetchPuzzle = async () => {
      ...
      try {
        ...
        let puzzleData: PuzzleData | null =
          initialPuzzle && initialPuzzle.id === pid ? initialPuzzle : null;

        if (!puzzleData) {
          const response = await fetchWithTimeout(`/api/puzzles/${pid}`, undefined, 15000);
          if (!response.ok) {
            setError(response.status === 404 ? "Puzzle not found" : "Failed to fetch puzzle");
            return;
          }

          puzzleData = (await response.json()) as PuzzleData;
        }
        ...
        if (initialPuzzleContent && puzzleData.id === puzzleIdLocal) {
          setPuzzleContent(initialPuzzleContent);
        }

        // Load content + progress with timeouts so the UI never gets stuck on "Loading puzzle...".
        const contentFetch =
          initialPuzzleContent && puzzleData.id === puzzleIdLocal
            ? null
            : fetchWithTimeout(`/api/puzzles/${puzzleData.id}/content`, undefined, 20000);

        const [contentResult, progressResult] = await Promise.allSettled([
          contentFetch ? contentFetch : Promise.resolve(null),
          fetchWithTimeout(`/api/puzzles/${puzzleData.id}/progress`, undefined, 15000),
        ]);

        if (contentResult.status === 'fulfilled') {
          try {
            if (contentResult.value && contentResult.value.ok) {
    ...
    fetchPuzzle();
  }, [puzzleIdLocal, initialPuzzle, initialPuzzleContent, setPuzzleStore, setHintsUsed, setGridState, markStarted, markCompleted, freshStartRequested, storageScope, localProgressKey, loadLocalProgress, saveLocalProgress]);
